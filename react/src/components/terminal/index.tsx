 ...
 * @LastEditors: Archy
 * @LastEditTime: 2022-03-03 15:31:12
 * @FilePath: \arkgen\react\src\components\terminal\index.tsx
 * @description: 
 */
...
export const TerminalButton: React.FC<TerminalMenuProps> = (props) => {
  ...
  useEffect(() => {
    setSelectedKeys(visible ? ["terminal"] : [])
  }, [visible])
  ...
  return (
    <div>
      <Menu mode="inline" onClick={handleClick} className={`${props.collapsed ? 'sidermenu--collapsed' : ''}`} style={props.style} selectedKeys={selectedKeys}>
        <Menu.Item key="terminal" icon={<Icon component={terminalSvg} style={iconStyle} />}>
          {renderMenuItem('控制台')}
        </Menu.Item>
      </Menu>
    </div>
